...
export function getSheetAddr(
  ...
  if (branch[0] === null) {
    throw new Error(`Branch ${addr.branch} not found`)
  }
  ...
  if (step[0] === null) {
    throw new Error(`Step ${addr.step} not found`)
  }

  const sheet = findSheet(addr.item, step[0]!)

  if (!sheet) {
    throw new Error(`Sheet ${addr.item} not found`)
  }
      ...
      nextState = produce(state, (draft) => {
        ...
        draft.branches.push({
          ...newHistoryBranch(action.file ?? 'Table 1', [
            newHistoryStep(action.description, action.sheets),
          ]),
        })
