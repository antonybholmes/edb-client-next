...
export const MENU_CLS = cn(
  ...
  'disabled:opacity-50 data-[state=active]:bg-muted',
  'data-[state=active]:font-semibold overflow-hidden'
)

export const UNDERLINE_CLS = cn(
  FOCUS_RING_CLS,
  'inline-flex items-center justify-center whitespace-nowrap py-1 px-2',
  'disabled:opacity-50 data-[state=active]:border-theme border-b-2 border-transparent',
  'data-[state=active]:font-semibold overflow-hidden'
)
...
const tabVariants = cva(
  'flex data-[orientation=horizontal]:flex-row data-[orientation=vertical]:flex-col relative',
  {
    variants: {
      variant: {
        default: '',
...
interface ITabContentForceMountPanelsProps extends IClassProps {
  groupId: string
  tabs: ITab[]
}
...
export function TabContentForceMountPanels({
  groupId,
  tabs,
}: ITabContentForceMountPanelsProps) {
  const { tabIndex } = useTabs(groupId)

  return tabs.map((tab, ti) => (
    <Activity mode={ti == tabIndex ? 'visible' : 'hidden'} key={ti}>
      {tab.content}
    </Activity>
  ))
}
