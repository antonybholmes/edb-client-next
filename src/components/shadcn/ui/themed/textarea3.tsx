import { cn } from '@lib/class-names'

import { BaseCol } from '@/components/layout/base-col'
import { forwardRef, useState, type TextareaHTMLAttributes } from 'react'
import type { IPlaceholderProps } from './input'
import { CONTAINER_CLS, PLACEHOLDER_CLS } from './input3'

export const TEXT_CLS =
  'px-1 relative w-full h-full text-foreground bg-background disabled:cursor-not-allowed disabled:opacity-50 outline-none ring-none resize-none custom-scrollbar'

export function Placeholder({
  id,
  placeholder,
  focus,
  value,
}: IPlaceholderProps) {
  return (
    <label
      className={PLACEHOLDER_CLS}
      data-focus={focus}
      data-value={value !== ''}
      style={{
        transform: `translateY(${focus || value ? '-1.05rem' : '0'})`,
        fontSize: `${focus || value ? '75%' : '100%'}`,
        fontWeight: `${focus || value ? '500' : 'normal'}`,
      }}
      htmlFor={id}
    >
      {focus ? placeholder?.replace('...', '') : placeholder}
    </label>
  )
}

export interface ITextAreaProps
  extends TextareaHTMLAttributes<HTMLTextAreaElement> {
  textCls?: string
}

export const Textarea3 = forwardRef<HTMLTextAreaElement, ITextAreaProps>(
  (
    { id, className, placeholder, value, disabled = false, textCls, ...props },
    ref
  ) => {
    const [focus, setFocus] = useState(false)
    //const [_value, setInputValue] = useState("")

    //function _onChange(event: ChangeEvent<HTMLTextAreaElement>) {
    //  setInputValue(event.target.value)
    //}

    //console.log(value)

    return (
      <BaseCol
        className={cn(CONTAINER_CLS, className)}
        onFocus={() => setFocus(true)}
        onBlur={() => setFocus(false)}
        data-enabled={!disabled}
        data-focus={focus}
      >
        {placeholder && (
          <Placeholder
            id={id}
            placeholder={placeholder}
            focus={focus}
            value={value}
          />
        )}

        <textarea
          id={id}
          className={cn(TEXT_CLS, textCls)}
          ref={ref}
          value={value}
          //onChange={_onChange}
          {...props}
        />
      </BaseCol>
    )
  }
)
Textarea3.displayName = 'Textarea3'
