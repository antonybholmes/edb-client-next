import type { VariantClass } from './lollipop-utils'
...
export interface ILollipopStats {
  position: number
  countMap: Record<VariantClass, Record<string, IAAData[]>>
}

export function newAAStats(position: number): ILollipopStats {
  return {
    position,
    countMap: {} as Record<VariantClass, Record<string, IAAData[]>>,
  }
}
...
): Record<VariantClass, Record<string, IAAData[]>> {
...
export function aaSet(
  ...
  if (!(variantClass in stats.countMap)) {
    stats.countMap[variantClass] = {} //new Map<string, Set<IAAData>>())
  }

  if (!(database in stats.countMap[variantClass]!)) {
    stats.countMap[variantClass]![database] = []
  }

  stats.countMap[variantClass]![database]!.push({
    sample,
    aa,
  })
}
