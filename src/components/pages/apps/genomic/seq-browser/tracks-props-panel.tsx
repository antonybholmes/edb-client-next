  ...
  return (
    <>
      ...
      {showDialog.id.includes('edit-seq') && (
        <SeqEditDialog
          ...
          callback={(group, track) => {
            console.log('update track group', group)
      ...
      {showDialog.id.includes('edit-cytobands') && (
        <CytobandsEditDialog onCancel={() => setShowDialog({ ...NO_DIALOG })} />
      )}
        ...
        <OKCancelDialog
          ...
          onResponse={r => {
            if (r === TEXT_OK) {
              ...
              let seqsByGroup = groups.map(g =>
                state.selected[g.id]
                  ? where(g.tracks, t => SEQ_TRACK_TYPES.has(t.type))
                  : []
              )

              let groups = seqsByGroup.filter(g => g.length > 0)
              ...
              seqsByGroup = groups.map(g =>
                state.selected[g.id]
                  ? where(g.tracks, t => BED_TRACK_TYPES.has(t.type)).map(
                      ti =>
                        groups[gid]!.tracks[
                          groups[gid]!.order[ti]!
                        ]!
      ...
      <PropsPanel className="gap-y-2 pr-1">
        <VCenterRow className="justify-between">
          <VCenterRow className="gap-x-1">
            {/* <VCenterRow className="w-6 justify-end"> */}
            ...
            <VCenterRow>
              ...
              {multiselect &&
                groups.filter(g => selectedGroups[g.id] ?? false).length >
                  0 && (
                  <>
                    <ToolbarSeparator />
