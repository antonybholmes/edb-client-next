...
function ExtGseaRankingSvg({ result }: { result: IExtGseaPlotResult }) {
...
function ExtGseaTitleSvg({
  ...
  if (!displayProps.title.show) return null
...
function ExtGseaEsSvgPlot({ result }: { result: IExtGseaPlotResult }) {
  const { plot } = useExtGseaContext()
  ...
  const esSvg = useMemo(() => {
    ...
    return esSvg
  }, [xax, yaxEs, result, plot])

  return esSvg
}

function ExtGseaGenesSvgPlot({ result }: { result: IExtGseaPlotResult }) {
  const { plot } = useExtGseaContext()
