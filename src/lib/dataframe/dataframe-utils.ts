...
export function log(
    ...
    switch (base) {
        case 2:
            return log2(df, add).setName(
                add > 0 ? `Log2(x + ${add})` : `Log2(x)`
            )

        case 10:
            return log10(df, add).setName(
                add > 0 ? `Log10(x + ${add})` : `Log10(x)`
            )
        ...
        .filter((c) => {
            if (g.columnNames) {
                ...
                for (const x of lcSearch) {
                    if (
                        g.columnNames[c.col % g.columnNames.length]?.includes(x)
                    ) {
                        return true
                    }
                }
